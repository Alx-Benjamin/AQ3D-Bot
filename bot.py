...
class BotApp:
    def __init__(self, root):
        ...
        self.location_frame.grid(row=1, column=0, padx=10, pady=10, sticky="nsew")

        self.skill_settings_frame = CollapsibleFrame(root, title="Skill Settings", width=frame_width)
        self.skill_settings_frame.grid(row=2, column=0, padx=10, pady=10, sticky="nsew")
        ...
        self.log_frame = CollapsibleFrame(root, title="Log")
        self.log_frame.grid(row=0, column=1, rowspan=5, padx=10, pady=10, sticky="nsew")

        # --- Status Frame ---
        self.aq3d_status_label = ctk.CTkLabel(self.status_frame.content_frame, text="Not Running")
        ...
        ctk.CTkButton(self.location_frame.content_frame, text="Set Enemy Health Location",
                     command=self.set_health_location).grid(row=1, column=0, padx=5)

        self.player_health_location_label = ctk.CTkLabel(self.location_frame.content_frame,
                                                        text=f"Player Health Location: Not Set")
        self.player_health_location_label.grid(row=2, column=1, padx=5)
        ctk.CTkButton(self.location_frame.content_frame, text="Set Player Health Location",
                     command=self.set_player_health_location).grid(row=2, column=0, padx=5)

        self.loot_location_label = ctk.CTkLabel(self.location_frame.content_frame, text=f"Loot Button Location: Not Set")
        self.loot_location_label.grid(row=3, column=1, padx=5)
        ctk.CTkButton(self.location_frame.content_frame, text="Set Loot Button Location",
                     command=self.set_loot_button_location).grid(row=3, column=0, padx=5)

        self.detect_revive_location_label = ctk.CTkLabel(self.location_frame.content_frame,
                                                        text=f"Detect Revive Location: Not Set")
        self.detect_revive_location_label.grid(row=4, column=1, padx=5)
        ctk.CTkButton(self.location_frame.content_frame, text="Set Detect Revive Location",
                     command=self.set_detect_revive_location).grid(row=4, column=0, padx=5)

        self.revive_button_location_label = ctk.CTkLabel(self.location_frame.content_frame,
                                                        text=f"Revive Button Location: Not Set")
        self.revive_button_location_label.grid(row=5, column=1, padx=5)
        ctk.CTkButton(self.location_frame.content_frame, text="Set Revive Button Location",
        ...
        ctk.CTkLabel(self.skill_settings_frame.content_frame, text="Skill").grid(row=0, column=0, padx=5)
        ctk.CTkLabel(self.skill_settings_frame.content_frame, text="Hotkey").grid(row=0, column=1, padx=5)
        ctk.CTkLabel(self.skill_settings_frame.content_frame, text="Cooldown").grid(row=0, column=2, padx=5)
        ctk.CTkLabel(self.skill_settings_frame.content_frame, text="Toggle Skill").grid(row=0, column=3, padx=5)
        for i, skill_name in enumerate(self.skill_names):
            ctk.CTkLabel(self.skill_settings_frame.content_frame, text=f"{skill_name}:").grid(row=i + 1, column=0, padx=5, pady=2)

            skill_entry = ctk.CTkEntry(self.skill_settings_frame.content_frame, width=50)
            ...
            cooldown_entry = ctk.CTkEntry(self.skill_settings_frame.content_frame, width=50)
            ...
            skill_enabled_checkbox = ctk.CTkCheckBox(self.skill_settings_frame.content_frame, text="Enabled",
        ...
        # --- Settings Frame ---
        self.settings_frame.content_frame.grid_rowconfigure(0, weight=1) 
        self.settings_frame.content_frame.grid_rowconfigure(1, weight=1) 
        self.settings_frame.content_frame.grid_rowconfigure(2, weight=1) 
        self.settings_frame.content_frame.grid_rowconfigure(3, weight=1)
        self.settings_frame.content_frame.grid_columnconfigure(0, weight=1) 
        self.settings_frame.content_frame.grid_columnconfigure(1, weight=1)
        self.settings_frame.content_frame.grid_columnconfigure(2, weight=1)

        ctk.CTkLabel(self.settings_frame.content_frame, text="Movement Loops:").grid(row=0, column=0, sticky="ew")
        self.movement_loops_entry = ctk.CTkEntry(self.settings_frame.content_frame)
        self.movement_loops_entry.grid(row=0, column=1, sticky="ew")
        self.movement_loops_entry.insert(0, str(self.movement_loops))

        ctk.CTkLabel(self.settings_frame.content_frame, text="Potion Hotkey:").grid(row=1, column=0, sticky="ew")
        self.potion_hotkey_entry = ctk.CTkEntry(self.settings_frame.content_frame)
        self.potion_hotkey_entry.grid(row=1, column=1, sticky="ew")
        self.potion_hotkey_entry.insert(0, self.potion_hotkey)

        ctk.CTkLabel(self.settings_frame.content_frame, text="No Enemy Timeout (minutes):").grid(row=2, column=0, sticky="ew")
        self.no_enemy_timeout_entry = ctk.CTkEntry(self.settings_frame.content_frame)
        self.no_enemy_timeout_entry.grid(row=2, column=1, sticky="ew")
        self.no_enemy_timeout_entry.insert(0, str(self.no_enemy_timeout_minutes))

        ctk.CTkLabel(self.settings_frame.content_frame, text="Max Runtime (hours, 0=forever):").grid(row=3, column=0, sticky="ew")
        self.max_runtime_entry = ctk.CTkEntry(self.settings_frame.content_frame)
        ...
        self.timer_label = ctk.CTkLabel(self.log_frame.content_frame, text="Bot Runtime: 00:00:00")
    ...
    def save_movement_keys(self):
        for key in self.movement_keys:
            self.movement_keys[key] = self.movement_key_vars[key].get()
        self.settings_modified = True
        self.log(f"Movement keys updated: {self.movement_keys}")
    ...
    def save_all_settings(self):
        ...
        settings = {
            ...
            'movement_keys': self.movement_keys,  # Save the dictionary
            ...
            'stop_bot_on_death': self.stop_bot_on_death.get(),  # Save the new setting
    ...
    def load_settings(self):
        if os.path.exists('settings.json'):
            ...
            self.stop_bot_on_death.set(settings.get('stop_bot_on_death', False))  # Load the new setting
    ...
    def update_gui_elements_from_settings(self):
        ...
        self.max_runtime_entry.insert(0, str(self.max_runtime_hours))
        # Update movement key checkboxes
        for key, enabled in self.movement_keys.items():
            self.movement_key_vars[key].set(enabled)
    ...
    def bot_loop(self):
        while self.bot_running:
            ...
            if self.check_and_handle_death():
                if self.stop_bot_on_death.get():  # Check the new setting
    ...
    def check_and_handle_death(self):
        if not self.is_player_alive():
            if self.is_revive_button_present():
                ...
                time.sleep(1)  # Give time for the revive to take effect
                return True # Player was dead, and revived
            else:
                self.log("Player health low, using potion")
                pyautogui.press(self.potion_hotkey)
                time.sleep(1)
                return False # Player was not fully dead.
        else:
            self.log("Player health is good, continue fighting")
            return False # Player was not dead.
    ...
    def is_revive_button_present(self):
        if self.detect_revive_box:
            ...
            average_color = self.get_average_color(screen)
            target_color = (88, 0, 0)  # Target color for the revive button
            tolerance = 10
            ...
            if self.is_color_within_tolerance(average_color, target_color, tolerance):
                ...
                pyautogui.press('esc')
                time.sleep(1)  # Give time for the ESC action to take effect
                screen = ImageGrab.grab(bbox=self.detect_revive_box)
                average_color = self.get_average_color(screen)
    ...
    def move_randomly(self):
        ...
        for _ in range(random.randint(1, self.movement_loops)):
            # Get a list of enabled movement keys
            enabled_keys = [key for key, enabled in self.movement_keys.items() if enabled]
            if enabled_keys:
                button = random.choice(enabled_keys)
